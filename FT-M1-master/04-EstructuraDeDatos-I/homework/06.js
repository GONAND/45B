...
const ordenarPorAño = (catalogo, nuevasPeliculas) => {
  let nuevaLista = catalogo.concat(nuevasPeliculas);
  if (nuevaLista.length <= 1) return nuevaLista;
  let pivot = Math.floor(nuevaLista.length / 2);
  let left = [];
  let right = [];
  for (let i = 0; i < nuevaLista.length; i++) {
    if (i !== pivot) {
      if (nuevaLista[i].añoDeEstreno <= nuevaLista[pivot].añoDeEstreno) {
        left.push(nuevaLista[i]);
      } else {
        right.push(nuevaLista[i]);
      }
    }
  }
  let result = ordenarPorAño(left)
    .concat(nuevaLista[pivot])
    .concat(ordenarPorAño(right));
  return result;
};
