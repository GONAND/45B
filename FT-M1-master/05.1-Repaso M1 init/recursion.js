...
function ordenarPedidos(pedidos) {
  // Tu código aquí:
  listaOrdenada = [];
  for (const key in pedidos.pedido) {
    if (pedidos.pedido[key] < pedidos.pedido[key + 1]) {
      listaOrdenada.push(pedidos.pedido[key]);
    }
  }
  return listaOrdenada;
}
let pedidos = { nombre: "martin" };
console.log(ordenarPedidos(200));
